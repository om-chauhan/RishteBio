...
const TEMPLATES: TemplateConfig[] = [
  { id: TemplateType.TRADITIONAL, name: 'Traditional', thumbnail: 'https://picsum.photos/300/400?random=1', description: 'Classic and formal layout.' },
  { id: TemplateType.MODERN, name: 'Modern', thumbnail: 'https://picsum.photos/300/400?random=2', description: 'Clean lines with a side profile.' },
  { id: TemplateType.MINIMAL, name: 'Minimal', thumbnail: 'https://picsum.photos/300/400?random=3', description: 'Elegant typography focused.' },
  { id: TemplateType.CREATIVE, name: 'Creative', thumbnail: 'https://picsum.photos/300/400?random=4', description: 'Stylish and colorful design.' },
];
      ...
      <div className={`flex flex-col lg:flex-row h-full overflow-hidden ${showDraftModal ? 'blur-sm pointer-events-none' : ''} pb-14 lg:pb-0 print:h-auto print:overflow-visible`}>
          ...
          <div className="flex border-b border-slate-200 bg-slate-50 overflow-x-auto scrollbar-hide">
            {tabs.map(tab => (
              <button
                ...
                className={`flex-1 flex flex-col items-center justify-center py-3 px-4 min-w-[80px] text-xs font-medium transition-colors ${
                  activeTab === tab.id 
        ...
        <div ref={previewContainerRef} className={`${mobileView === 'editor' ? 'hidden lg:flex print:!block' : 'flex'} flex-grow bg-slate-200 flex-col h-full relative print:bg-white print:h-auto print:overflow-visible print:static min-w-0`}>
          ...
          <div className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-3 md:px-6 shadow-sm no-print z-10 gap-2 flex-shrink-0 w-full overflow-x-hidden">
            ...
            <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide mask-linear-fade flex-1 sm:flex-none">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider hidden sm:inline">Template</span>
              <div className="flex bg-slate-100/80 p-1 rounded-lg border border-slate-100">
                {TEMPLATES.map(t => (
                  <button
                    key={t.id}
                  ...
                  >
                    {data.templateId === t.id && <CheckCircle size={10} strokeWidth={3} />}
                    {t.name}
                  </button>
                ))}
              </div>
